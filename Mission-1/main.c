    ...
    while (run) {
        ...
        switch (state) {
            ...
            case IDLE:
                ...
                else if (get_ACCL() == 1) next_state = LAUNCH;
                break;

            case TESTING:
                if (get_TESTS() == 1)  next_state = READY;
                else if (get_TESTS() == 0) next_state = INIT;
                break;
...
int get_ACCL() {
    int val;
    printf("\nLaunch Accel Detected? (1=Yes, 0=No): ");
    scanf("%d", &val);
    return val;
}
...
int get_PRESSURE() {
    int val;
    printf("\nPressure Drop Detected? (1=Yes, 0=No): ");
    scanf("%d", &val);
    return val;
}
...
int get_RECOVERED() {
    int val;
    printf("\nRecovery button pressed? (1=Yes, 0=No): ");
    scanf("%d", &val);
    return val;
}
